...
public class O2TankTriggerHandler : MonoBehaviour
{
    ...
    private void Awake()
    {
        // Get the O2TankBoxSpawner script from the same GameObject
        ShellBoxSpawner = GetComponent<ShellBoxSpawner>();
        if (ShellBoxSpawner == null)
        {
            Debug.LogError("O2TankBoxSpawner script not found on the same GameObject.");
        }
    }
    ...
    private void HandleO2TankCollision(GameObject o2TankObject)
    {
        // Add your logic here for handling the o2tank collision
        Debug.Log($"Handling logic for {o2TankObject.name}.");
    }
}
