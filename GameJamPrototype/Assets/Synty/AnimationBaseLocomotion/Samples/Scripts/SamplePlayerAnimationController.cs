    ...
    public class SamplePlayerAnimationController : MonoBehaviour
    {
        ...
        private void ActivateAim()
        {
            if (!_isAiming)
            {
                ...
                _animator.SetTrigger("AimStart"); // Set trigger instead of a bool
            }
        }
        ...
        private void DeactivateAim()
        {
            if (_isAiming)
            {
                ...
                _animator.SetTrigger("AimEnd"); // Set trigger to end aiming
            }
        }
        private void HandleAimingInput()
        {
            ...
            {
                isAimingInput = Input.touchCount > 0 && Input.GetTouch(0).phase == TouchPhase.Stationary;
            }

            // Toggle aiming based on input
            if (isAimingInput)
            {
                if (!_isAiming) // Only trigger if not already aiming
                {
                    ActivateAim();
                }
            }
            ...
            {
                if (_isAiming) // Only deactivate if currently aiming
                {
                    DeactivateAim();
                }
            }
        ...
        {
            // Only apply movement-based rotation if the player is not aiming
            if (_isAiming)
            {
                // Let the PlayerAiming script handle rotation when aiming
                return;
            }
